...
const txt2woff = (game, text) => {
  const key = `__game__${game}__woff__${md5(text)}`;

  return new Promise((resolve, reject) => {
    const cachedData = woffCache.get(key);
    if (cachedData) {
    ...
    fontmin.run((err, files) => {
      if (err) {
        ...
        const fontFile = files.find((f) => f.contents);
        if (fontFile) {
          const woff = b2a(fontFile.contents);
          woffCache.set(key, woff); // 快取結果
          resolve(woff);
        } else {
          reject(new Error("Expected font file not found in Fontmin output."));
        }
      } else {
