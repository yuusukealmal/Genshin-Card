...
}).toString(); // this will break if hoyolab starts to tie tokens to user agents
    ...
    if (cachedData) {
      ...
      http({
        ...
        headers: {
          ...headers,
          Cookie: `${
            process.env.HOYOLAB_TOKENV2 == "true" ? "ltoken_v2" : "ltoken"
          }=${process.env.HOYOLAB_TOKEN};${
            process.env.HOYOLAB_TOKENV2 == "true" ? "ltuid_v2" : "ltuid"
          }=${process.env.HOYOLAB_ID};`, // HoYoLAB only cares about the LToken and LTUID cookies
          DS: getDS(),
        },
      })
        .then((resp) => {
                  ...
                  switch (game) {
                        ...
                        const percentage = Math.min(
                          (
                            (world_explorations.reduce(
                              (total, next) =>
                                total + next.exploration_percentage,
                              0
                            ) /
                    ...
                    case "zzz":
                      if (detail) {
                        ...
                        const commemorative_coins =
                          commemorative_coins_list[0].num;
                        const data = {
                          uid: game_role_id,
                          commemorative_coins,
                          ...resp.data.stats,
